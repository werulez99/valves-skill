# Attack Thesis Formation Methodology

> Extracted from attack-thesis.md (v1.7.0-PATCH12).
> Used by: chain, depth phases.

---

## Purpose

The attack thesis is the audit's running commitment to a small, concrete set of attack paths. Three versions are produced through the audit: **v1** (post-classification, pre-depth), **v2** (post-depth iter 1 + Economic + Propagation), **v3** (post-verification).

## v1 — Initial Thesis Formation

**Inputs**: findings inventory, strongest exploit cards (custody-loss card winners become candidate thesis paths unless refuted), finding classification (flat BC tags per finding, NOT clusters), design context, attack surface.
...
**Rules for v1**:
- Use BC tags from finding classification to *identify* related findings (same BC-NNN tag = likely same class), but do NOT assume they are the same bug -- depth has not confirmed yet.
...
## v2 — Post-Depth Refinement

**Inputs**: previous attack thesis (v1), all depth/economic/propagation/diff-audit outputs, confidence scores.
...
- Confidence: 0.00-1.00 (composite)
- v1 commit: {timestamp}
- v2 commit: {timestamp}
```

## v3 — Post-Verification Final Adjudication

**Inputs**: previous attack thesis (v2), verifier verdicts, verification inheritance, negative results.

**v3 status rules** (apply per path):

| v2 status | Verification outcome on supporting findings | v3 status |
|---|---|---|
...
**PRIOR_NEGATIVE handling**: if a path was refuted in a prior audit but the current audit produced `[POC-PASS]`, mark `PRIOR_NEGATIVE_OVERRIDDEN` and cite the prior NR entry. Conversely, if a v2 CANDIDATE path matches a prior NR entry verbatim and no fresh evidence exists, mark `PRIOR_NEGATIVE_INHERITED` and downgrade to REFUTED with the citation.

**Output**: v3 written, with v2 preserved as `## Appendix: v2` and v1 still in `## Appendix: v1`.
